...
const checkToken = async function (userId, socket, chat) {
  ...
  if (userInfoform.id && userInfoform.islogin) {
    ...
    if (socket.id !== userInfoform.id) {
      ...
      //   console.log(users, messageList, "更新++");
      //   console.log(messageList, "messageList111");
...
var chat = io.of("/chat").on("connection", function (socket) {
  ...
  socket.on(
    ...
    async (userInfo, toWho, message, sockedType = "everyone") => {
      ...
      if (sockedType === "someone") console.log(socket.id, userInfo.id);
      console.log(userInfo.id, socket.id);
      console.log(toWho.id, toWho.id);
      chat
        .to(socket.id)
        .to(toWho.id)
