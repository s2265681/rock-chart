...
let users = []; // 记录当前所有用户信息 { avatar: "",id: "", username: ""}
...
io.on("connect", (socket) => {
  ...
  socket.on("to send msg", (userInfo, toWho, message, sockedType='everyone') => {
    let newMsg = {
      ...userInfo,
      toWhoId: toWho.id,
      toWho,
    ...
    if(sockedType === 'everyone')io.emit("receviedMessage", toWho, messageList);
    if(sockedType === 'someone')io.to(userInfo.id).to(toWho.id).emit("receviedMessage", toWho, messageList);
  });
